...
const loginRouter = (db) => {
  router.post('/', (req, res) => {
    ...
    } else if (validFirstname(firstname) && validLastname(lastname)) {
      console.log('Login: Trying admin login');
      db.selectWithWhereCondition(userTable, { firstname, lastname }).then((result) => {
        const user = result && result.length && result[0].isAdmin ? result[0] : null;
        if (user) {
          res.status(200).json({
            success: true,
            id: user.id,
